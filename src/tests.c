...
/**
 * acc_element contains the computed data, element is the array element to get the data from.
 */
...
static ArrayElement* allocate_number(void* number)
{
    ArrayElement* element = calloc(sizeof(struct ArrayElement), 1);
...
static void free_number(ArrayElement* element)
{
    if (element) {
        if (element->data) {
            free(element->data);
